...
class Model(TorchModelV2, nn.Module):
    def __init__(
        ...
        self.action_encoder = nn.Sequential(
            SlimConv2d(obs_shape[0], 8, 3, 2, 1),
            SlimConv2d(8, 8, 3, 2,  1),
            SlimConv2d(8, 8, 3, 2,  1),
            nn.Flatten(),
            nn.ReLU(),
            nn.Linear(128, self.encoder_out_features),
            nn.ReLU(),
        )
        ...
        self.action_individual = nn.Sequential(
            nn.Linear(self.encoder_out_features * 1, 64),
            nn.ReLU(),
            nn.Linear(64, self.shared_nn_out_features_per_agent * 1),
            nn.ReLU(),
        )
        post_logits = [
            nn.Linear(2 * self.shared_nn_out_features_per_agent, 32),
            nn.ReLU(),
        ...
        self.value_encoder = nn.Sequential(
            nn.Conv2d(obs_shape[0], 1, 3, stride=1),
            nn.Flatten(),
            nn.ReLU(),
            nn.Linear(529, self.encoder_out_features),
            nn.ReLU(),
        )
...
#        else:
        ...
        action_individual_features = torch.empty(
            batch_size, self.n_agents, self.shared_nn_out_features_per_agent
        ).to(device)
        for i in range(self.n_agents):
            action_individual_features[:, i] = self.action_individual(
...
#        print("actionsharedfeatures", action_shared_features.shape)
#        print("actionindivisualfeatures", action_individual_features.shape)
#
        actionfeats = torch.cat((action_shared_features, action_individual_features), axis=2)
#        print("sharedconcatenated", actionfeats.shape)

        for i in range(self.n_agents):
            outputs[:, i] = self.action_output(actionfeats[:, i])
